...
通知前に商品名・価格・容量の表記揺れを正規化します。2026-09-12の実機では `元=114件 / 重複整理後=67件` まで削減できた後、さらに `3食入/3食`、`6切入、1パック/6切入 1パック` のような表記を同一視する改善を入れました。

曖昧な商品名をまとめる場合は安全のため、**同価格・容量矛盾なし・同じ元画像・同じ対象期間** を必須にします。これによりサーモン系などの表記揺れをまとめつつ、別画像・別期間の商品を誤って統合しにくくします。Notion行は削除せず、通知表示だけを整理します。
