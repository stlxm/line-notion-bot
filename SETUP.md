...
このDBが未設定でも、Phase 2の`今日使える / ペース / 予算提案 / 異常支出 / 年間予測 / 月締め / 月次レビュー`は利用できます。使えないのは貯金目標機能だけです。
...
`月締め`は前月をプレビューします。任意月は`月締め YYYY-MM`。
...
月別管理DBの`締め済み / 締め日時 / 確定支出`を更新します。プレビューだけではDBを書き換えません。

カード未処理が残っている場合は警告しますが、明示的な`月締め確定`は実行可能です。

## 予算提案

過去3か月の支出データを参照し、平均+約5%の余裕を提案します。自動で予算DBは書き換えません。

## 異常支出

過去約4か月の支出中央値と比較します。高額な予定支出も候補に出るため、異常=誤登録とは扱いません。

## 貯金目標

追加後は`貯金更新 目標名 現在額`で進捗を更新します。期限がある場合は、残額と期限から月あたり必要額の目安を表示します。
...
```text
月別管理DBへ 締め済み / 締め日時 / 確定支出 を追加
貯金目標を使うなら貯金目標DB作成
NOTION_SAVINGS_GOALS_DATABASE_IDをRenderへ設定
Renderを最新mainで再デプロイ
```
