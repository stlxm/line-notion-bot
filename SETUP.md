...
カードカテゴリには、未処理・自動分類に加えて「店名の小書き文字補正を学習する」自動機能があることも表示します。
...
追加の環境変数やNotion DBは不要です。既存の `NOTION_CARD_RULES_DATABASE_ID` を利用します。

カード未処理で「店名を変更する」を選び、変更が次のような**大きい仮名から対応する小書き仮名への変更だけ**なら、自動的に補正ルールとして学習します。
...
次回以降、カード明細から同じ旧表記が来た場合は、未処理DBへ保存する前に修正版へ置換します。

安全のため、店名の追加・削除・小書き文字以外の変更が混ざる場合は学習・過去一括修正を行いません。その場合は今までどおり現在のカード1件だけ店名変更します。
