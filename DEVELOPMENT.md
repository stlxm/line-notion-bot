...
状態: **取得・5ID同期は実機確認済み / 強化重複整理後のLINE通知は要実機確認**
...
## 2026-09-12 重要修正4 — 表記揺れ重複をさらに統合

実機ログでは `元=114件 / 重複整理後=83件` まで減ったが、以下のような同一商品らしい重複が残った。
...
価格が違うものは別商品
容量/個数が両方明確で違うものも別商品
商品名が包含関係または同一商品ファミリーなら同一候補
短期間・「限り」備考・詳細情報を持つ候補を残す
```
...
4. ログの [Notion今日分] 元=N件 / 重複整理後=M件 を確認
5. LINE先頭に「🔥 今日・短期特売」が出ることを確認
6. なす/ミニトマト/サーモン/豚かたロース等の表記揺れ重複がさらに減っていることを確認
7. 350ml×24 と 500ml×24 等の別容量は別商品として残ることを確認
8. 月間・長期特売が後半に出ることを確認
