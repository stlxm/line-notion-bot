...
def command_list_text():
    return (
        "【コマンド一覧】\n"
        "家計: 本日のレポート / 今月 / 今日使える / ペース / 予算一覧 / 予算 100000 / 予算 お菓子 3000 / 予算提案 / 異常支出 / 年間予測 / 月締め / 月次レビュー / 週次レポート\n\n"
        "入力: 自然文入力 / 今日サミットで2380円使った / 支出テンプレート / 支出 金額 店名\n\n"
        "修正: 直前登録 / 直前修正 / 直前取り消し\n\n"
        "貸し借り: 貸した 相手 金額 [メモ] / 借りた 相手 金額 [メモ] / 貸し借り一覧 / 精算 相手 金額\n\n"
        "カード: カード未処理 / カード自動登録 / カードテスト\n\n"
        "記録: メモ 内容 / メモ一覧 / メモ削除 / 固定費一覧 / 固定費追加 / データ追加\n\n"
        "目標: 貯金目標 / 貯金目標追加 / 貯金更新\n\n"
...
def suggest_for_intent(text):
    ...
    groups = [
        ...
        (["貸し", "借り", "立替", "返して", "返す"], [("貸し借り一覧", "未精算の貸し借りを確認"), ("貸した 相手 金額", "貸したお金を記録"), ("借りた 相手 金額", "借りたお金を記録"), ("精算 相手 金額", "返済・返金済みにする")]),
        (["旅行", "貯金", "目標", "ためたい", "貯めたい"], [("貯金目標", "目標と進捗を確認"), ("年間予測", "現在ペースの年間支出を確認"), ("予算提案", "予算の目安を作る")]),
        (["カード", "クレカ", "明細", "未処理"], [("カード未処理", "未分類カードを整理"), ("カード自動登録", "自動分類ルールを確認")]),
        (["メモ", "忘れ", "todo", "やること"], [("メモ 内容", "新しいメモを保存"), ("メモ一覧", "保存中のメモを確認")]),
        (["予算", "月末", "振り返", "レビュー"], [("予算一覧", "今月の予算状況を確認"), ("予算提案", "次回予算の候補を確認"), ("月次レビュー", "今月をAIで振り返る"), ("月締め", "前月を締める前に確認")]),
        (["ai", "分析", "質問", "聞きたい"], [("AI 質問内容", "Notionや家計簿の内容をAIに質問"), ("AI Model", "使用中モデルを確認")]),
    ]
