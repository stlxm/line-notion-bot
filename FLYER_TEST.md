...
このテストはGemini APIを呼ばないので、無料枠を消費しません。
...
Shufooは個別リンクだけでなく画像URLにも配信IDを含みます。
...
[ ] 9783726841844 が含まれる
[ ] 4441736841834 が含まれる
[ ] 3487936841840 など別配信IDも別件として出る
[ ] 1件だけに潰れない
```

ここが正しくなるまではGemini解析へ進みません。
...
```text
[ ] 配信IDごとに別オブジェクト
[ ] 各IDには原則そのIDを含む画像URLだけが入る
[ ] 月間チラシと週次/その他チラシが混ざらない
...
```text
[ ] testSummitShufooDeliveryIds で複数配信IDを正しく検出
[ ] 9783726841844 / 4441736841834 等を別チラシとして認識
[ ] 各IDの画像と商品情報が対応
