...
2026-09-12の実機ログで、産地付き/なし・表記揺れ・OCR誤字による同一商品の重複がまだ残ることを確認したため、通知前の重複判定を強化しました。価格が同じで、容量/個数が矛盾せず、商品名が包含関係または同一商品ファミリーと判断できる場合は1件へまとめます。Notionの元データは削除・変更しません。
