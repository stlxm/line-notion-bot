...
以前は一部の先頭ボタンだけ `primary` になっていたため、LINE上で先頭1〜2個だけ緑色になり、機能の優先順位と関係なく強調されて見える問題がありました。

現在は次のルールに統一しています。

- メインメニューの操作ボタン: すべて `secondary`
- ジャンル・支払方法など同格の選択肢: すべて `secondary`
- カード通知のジャンル選択: すべて `secondary`
- 色による区別はセクション見出しのアクセントに限定
- ボタンの順番だけを理由に緑色にしない

これにより、先頭項目だけが不自然に強調されず、メニュー全体の見た目が均一になります。
