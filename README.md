...
# 「この機能ある？」案内 + Gemini補助判定 + 機能追加要望

状態: **実装済み・要Render設定/実機確認**

LINEで機能を確認すると、まず従来どおり登録済みの機能名・別名から高速判定します。そこに一致しない場合だけGeminiが「今ある機能の組み合わせで実現できそうか」を確認します。
...
3. Geminiが既存機能で可能と判断 → その機能の使い方を返す
4. 正式ロードマップ済み → 未実装であることと予定を返す
5. Geminiも「ない」と判断 → 入力された機能名をそのまま機能追加要望DBへ登録
```

Geminiが一時的に失敗した場合は「ない」と決めつけず、誤登録防止のため要望DBへは入れません。

`機能確認` だけ送った場合は、確認したい機能名も一緒に送る例を返します。
...
既定はLiteです。機能確認の補助判定も現在選択中のAIモデルを使いますが、登録済み機能に直接一致する場合はGeminiを呼びません。
