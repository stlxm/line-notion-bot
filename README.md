...
未処理カードが0件でもPhase 1を試せるように、管理者向けのテストコマンドがあります。

```text
カードテスト
```

送るたびに、実際のカード利用とは無関係な`Phase1テストショップ`の未処理カードを1件だけ作成し、そのまま通常のカード分類画面を表示します。Gmailやカード会社メールには触れません。

テストデータは通常の未処理と同じ扱いなので、ジャンルを保存すると家計簿・カード学習ルールDBへ記録されます。不要なテストデータは`登録しない`で消せます。
