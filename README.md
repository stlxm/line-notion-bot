...
同じ店舗でも配信IDが違えば別チラシです。GASは一覧ページから配信IDを列挙し、各配信IDのページを個別取得してGemini解析します。これにより月間・週次・日替わりなど複数種類のチラシを1つに混ぜません。
...
gas/FlyerLifeCalendar.gs
  → Shufoo配信ID検出、配信IDごとの個別解析、確認待ち登録、生活カレンダー同期、確認済み通知
```
