...
2026-09-12の実機ログでは、通知対象114件が第一段階の重複整理で67件まで減少しました。さらに `3食入/3食`、`6切入、1パック/6切入 1パック` のような容量表記揺れを正規化します。

曖昧な商品名を同一扱いする場合は誤統合防止のため、原則として **同じ価格・容量に矛盾なし・同じ元画像・同じ対象期間** を必須条件にします。完全に正規化後の商品名が一致する場合は、短期条件を優先してまとめます。Notionの元データは削除・変更しません。
